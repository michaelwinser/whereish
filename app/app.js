...
/* global handleAcceptRequest, handleDeclineRequest, handleAddContact, handleRemoveContact, handlePermissionChange */
...
(function() {
    ...
    // Track pending OAuth user for PIN setup (used in PIN modal flow)
    // TODO: Use this when PIN setup modal is implemented
    let pendingOAuthUser = null; // eslint-disable-line no-unused-vars
        ...
        return rawContacts.map(contact => {
            ...
            if (contact.encryptedLocation && identity) {
                try {
                    ...
                    console.debug('[v2] Failed to decrypt location for', contact.name, decryptErr.message);
                }
            }
